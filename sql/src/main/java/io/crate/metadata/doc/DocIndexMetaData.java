...
public class DocIndexMetaData {
    ...
    private ImmutableList<AnalyzedCheckConstraint> checkConstraints;
        ...
        if (null != metaMap) {
            ...
            if (null != checkConstraintsMap) {
                ...
                for (Map.Entry<String, String> entry : checkConstraintsMap.entrySet()) {
                    ...
                    Expression expr = SqlParser.createExpression(entry.getValue());
                    Function func = (Function) exprAnalyzer.convert(expr, analysisCtx);
                    checkConstraintsBuilder.add(new AnalyzedCheckConstraint(name, expr, func));
                }
            }
        }
